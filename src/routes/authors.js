...
router.get("/:authorid", async function(req, res, _){
...
router.patch("/:authorid", async function(req, res, _){
    ...
    return authorsController.update({id: authorID, name: name, citation_name: citation_name, biography: biography})
    .then((response) => res.status(200).json(response))
    .catch((e) => {
         log.error(e);
         return res.status(400).json({error: "Error when updating author"});
    });
});
